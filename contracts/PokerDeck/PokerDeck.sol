...
contract PokerDeck is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public mintFee = 50 ether; // fee is 50 HAMSTER
    uint256 public teamMintLimit = 0;
    ...
    function teamMint() public nonReentrant onlyOwner {
        uint256 tokenId = totalSupply();

        teamMintLimit++;
        require(teamMintLimit <= 100, "Error: exceeds team mint limit");

        _mintPoker(msg.sender, tokenId);
    }

    function mint() public payable nonReentrant {
        require(msg.value == mintFee, "Error: mint fee must be 50 HAMSTER");
        ...
        require(
            tokenId <= 8888,
            "Error: supply for buy is limited at 8,889 POKER"
        );
